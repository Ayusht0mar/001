...
export async function handlePaymentEvent(payload: any) {
  ...
  try {
    console.log("[handlePaymentEvent] Incoming payload:", JSON.stringify(payload));
    ...
    if (!subscription) {
      console.warn(`[handlePaymentEvent] Subscription not found for providerSubscriptionId: ${data.subscription_id}`);
      // Optionally, you could create a payment with null subscriptionId or log for further investigation
      return;
    }
    ...
    await prisma.$transaction(async (tx) => {
      try {
        ...
        console.log(`[handlePaymentEvent] Payment upserted:`, payment);
        ...
        if (payment.status === "SUCCESS") {
          ...
          console.log(`[handlePaymentEvent] Subscription promoted to ACTIVE for id: ${subscription.id}`);
        }
      } catch (err) {
        console.error("[handlePaymentEvent] Error in payment upsert transaction:", err);
        throw err;
      }
    });
  } catch (err) {
    console.error("[handlePaymentEvent] Handler error:", err);
    throw err;
  }
}
